...
{	
	...
	// NyrasPrologueDemake: warning and an angry face
	if (Npc_IsPlayer(other) == true)
	{
		// In case of Kirgo do it only, if it is not a duel
		if (GRD_251_Kirgo == Hlp_GetInstanceID(self))
		{
			if (Kirgo_FirstDuelFinished == false)
			{
			} else if (Kirgo_SecondDuelFinished == false)
			{
				if (Kirgo_PlayerIsReady == false)
				{
...
//						// B_Say				(self,	other,	"$RUNCOWARD");	
		...
		&&	(C_GetAttackReason(self)!=AIV_AR_NONE) )
		{
...
//						// B_Say				(self,	other,	"$RUNCOWARD");	
	...
	// NyrasPrologueDemake: NPCs don't flee
	//-------- Flucht antreten? --------
	/*
...
//			B_Say			(self, 	other,	"$LETSFORGETOURLITTLEFIGHT");
		...
		*/
		...
		// NyrasPrologueDemake: If player attacked him before talking and won't win, will be killed.
		if (GRD_251_Kirgo == Hlp_GetInstanceID(self))
		&& (Npc_KnowsInfo(hero, DIA_GRD_251_Kirgo_Hello) == true)
		&& (((Kirgo_FirstDuelFinished == true) && (Kirgo_FirstDuelTalked == false))
		|| ((Kirgo_SecondDuelFinished == true) && (Kirgo_SecondDuelTalked == false)))
		{
	...
	//-------- Plündern ! --------
	if (!C_NpcIsBoss(self) && C_NpcIsHuman(other) && (Npc_IsInState(other, ZS_Unconscious) || Npc_IsDead(other)) ) 
	{
