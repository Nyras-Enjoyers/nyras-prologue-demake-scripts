...
func void B_AssessSc ()
{
	...
	&&	(Npc_GetDistToNpc	(self, other) < PERC_DIST_DIALOG) ) 		
	{
		...
		// Ratford says the first speech as SVM, if sees player first
		if (ORG_818_Ratford == Hlp_GetInstanceID(self))
		{
			if (Npc_KnowsInfo(her, DIA_ORG_818_Ratford_Hello) == false)
			&& (Jorik_CorpseReached == false)
			{
				if (Ratford_WhatTheHellNotInSVM == false)
				&& (Ratford_WhatTheHellSaidInSVM == false)
				{
					// Hero turns to Ratford
					...
					AI_OutputSVM(self, hero, "$Ratford_CH0_None_Ratford_64245_1");
					AI_Wait(self, 0.1);

					// SVM said
					Ratford_WhatTheHellSaidInSVM = true;
